# 人格卦象映射系统技术架构文档

## 1. 架构设计

```mermaid
graph TD
    A[用户浏览器] --> B[React前端应用]
    B --> C[本地存储 LocalStorage]
    B --> D[CSV题库文件]
    B --> E[贝叶斯推理引擎]
    B --> F[卦象计算引擎]
    
    subgraph "前端层"
        B
        E
        F
    end
    
    subgraph "数据层"
        C
        D
    end
    
    subgraph "算法层"
        G[自适应测试算法]
        H[概率更新算法]
        I[卦象映射算法]
    end
    
    E --> G
    E --> H
    F --> I
```

## 2. 技术描述

* **前端**: React\@18 + TypeScript + Tailwind CSS + Vite

* **状态管理**: Zustand

* **数据可视化**: Chart.js / D3.js

* **图标库**: Heroicons

* **数据存储**: 本地LocalStorage + CSV文件

* **算法库**: 自研贝叶斯推理引擎

* **构建工具**: Vite + ESLint + Prettier

## 3. 路由定义

| 路由          | 目的               |
| ----------- | ---------------- |
| /           | 首页，展示系统介绍和测试入口   |
| /test       | 自适应测试页面，进行心理量表测试 |
| /result/:id | 测试结果页面，显示卦象分析    |
| /history    | 历史记录页面，查看过往测试结果  |
| /database   | 题库管理页面，浏览题目和参数   |
| /about      | 关于页面，开源协议和使用说明   |

## 4. 核心数据结构

### 4.1 扩展题目数据结构

```typescript
interface ExtendedQuestion {
  id: string;
  category: 'inner_motivation' | 'outer_behavior'; // 内在动机 | 外在行为
  text_zh: string; // 中文题目
  text_en: string; // 英文题目
  options: QuestionOption[];
  
  // 新增自适应测试属性
  difficulty: number; // 区分度 0-1
  discrimination_targets: string[]; // 主要区分的类型组合
  information_value: number; // 信息价值评分
  usage_count: number; // 使用次数统计
  success_rate: number; // 区分成功率
  
  // 题目分类标签
  subcategory: string; // 子分类（如：成就动机、安全动机等）
  complexity_level: 'simple' | 'medium' | 'complex'; // 复杂度等级
  test_phase: 'exploration' | 'discrimination' | 'confirmation' | 'any'; // 适用测试阶段
}

interface QuestionBank {
  version: string;
  total_questions: number;
  inner_motivation_questions: ExtendedQuestion[]; // 30题
  outer_behavior_questions: ExtendedQuestion[]; // 30题
  metadata: {
    last_updated: string;
    validation_status: string;
    performance_metrics: BankPerformanceMetrics;
  };
}

interface QuestionOption {
  id: string;
  text_zh: string;
  text_en: string;
  // 对8种类型的影响系数 (-1 到 1)
  impact_coefficients: {
    qian: number;    // 乾卦系数
    kun: number;     // 坤卦系数
    zhen: number;    // 震卦系数
    xun: number;     // 巽卦系数
    kan: number;     // 坎卦系数
    li: number;      // 离卦系数
    gen: number;     // 艮卦系数
    dui: number;     // 兑卦系数
  };
}
```

### 4.2 自适应测试核心接口

```typescript
interface AdaptiveTestSession {
  id: string;
  start_time: number;
  current_phase: 'exploration' | 'discrimination' | 'confirmation';
  questions_answered: number;
  target_questions: number;
  answered_question_ids: string[];
  phase_transitions: PhaseTransition[];
  confidence_history: ConfidenceSnapshot[];
  information_gain_history: InformationGainRecord[];
}

interface PhaseTransition {
  from_phase: string;
  to_phase: string;
  timestamp: number;
  trigger_reason: string;
  confidence_at_transition: number;
}

interface ConfidenceSnapshot {
  question_number: number;
  timestamp: number;
  inner_confidence: number;
  outer_confidence: number;
  overall_confidence: number;
  entropy: number;
}

interface InformationGainRecord {
  question_id: string;
  expected_gain: number;
  actual_gain: number;
  selection_reason: string;
}

interface AdaptiveAlgorithmConfig {
  exploration_questions: number; // 探索阶段题目数 (默认3)
  discrimination_questions: number; // 区分阶段题目数 (默认4)
  confirmation_questions: number; // 确认阶段题目数 (默认3)
  confidence_threshold: number; // 置信度阈值 (默认0.7)
  entropy_threshold: number; // 熵值阈值 (默认1.5)
  phase_transition_strategy: 'fixed' | 'adaptive'; // 阶段转换策略
  information_gain_weight: number; // 信息增益权重
  diversity_weight: number; // 多样性权重
}

interface BankPerformanceMetrics {
  average_discrimination_rate: number;
  question_usage_distribution: Record<string, number>;
  phase_effectiveness: Record<string, number>;
  convergence_speed: number;
  user_satisfaction_score: number;
}
```

### 4.3 测试状态数据结构

```typescript
interface TestSession {
  id: string;
  start_time: number;
  current_question_index: number;
  answers: TestAnswer[];
  // 当前概率分布
  probability_distribution: {
    inner_motivation: ProbabilityMap; // 内在动机概率
    outer_behavior: ProbabilityMap;   // 外在行为概率
  };
  is_completed: boolean;
}

interface TestAnswer {
  question_id: string;
  option_id: string;
  response_time: number; // 响应时间（毫秒）
  timestamp: number;
}

interface ProbabilityMap {
  qian: number;
  kun: number;
  zhen: number;
  xun: number;
  kan: number;
  li: number;
  gen: number;
  dui: number;
}
```

### 4.3 卦象结果数据结构

```typescript
interface HexagramResult {
  id: string;
  timestamp: number;
  session_id: string;
  
  // 主卦信息
  main_hexagram: {
    name_zh: string;
    name_en: string;
    upper_trigram: Trigram; // 上卦
    lower_trigram: Trigram; // 下卦
    hexagram_code: string;  // 64卦编码
  };
  
  // 分层分析结果
  analysis: {
    layer1_basic: BasicAnalysis;     // 第一层：基础分析
    layer2_detailed: DetailedAnalysis; // 第二层：爻位分析
    layer3_related: RelatedAnalysis;   // 第三层：关联卦象
  };
  
  // 概率分布快照
  final_probabilities: {
    inner_motivation: ProbabilityMap;
    outer_behavior: ProbabilityMap;
  };
}

interface Trigram {
  name_zh: string;
  name_en: string;
  symbol: string; // Unicode卦象符号
  element: string; // 五行属性
  attribute: string; // 卦象属性
}

interface BasicAnalysis {
  personality_type: string;
  core_motivation: string;
  behavior_pattern: string;
  strengths: string[];
  challenges: string[];
  compatibility: string; // 内外一致性评估
}

interface DetailedAnalysis {
  key_lines: LineAnalysis[]; // 关键爻位
  development_advice: string[];
  attention_points: string[];
}

interface LineAnalysis {
  position: number; // 爻位 1-6
  strength: number; // 强度 0-1
  interpretation: string;
}

interface RelatedAnalysis {
  complementary_hexagram: string; // 综卦
  opposite_hexagram: string;      // 错卦
  mutual_hexagram: string;        // 互卦
  fortune_analysis: string;       // 运势分析（娱乐性）
  ethical_reminder: string;       // 伦理提醒
}
```

## 5. 核心算法设计

### 5.1 增强版贝叶斯自适应测试算法

```typescript
class EnhancedBayesianAdaptiveTest {
  private questionBank: QuestionBank;
  private currentProbabilities: {
    inner_motivation: ProbabilityMap;
    outer_behavior: ProbabilityMap;
  };
  private testSession: AdaptiveTestSession;
  private algorithmConfig: AdaptiveAlgorithmConfig;
  
  constructor(questionBank: QuestionBank, config?: AdaptiveAlgorithmConfig) {
    this.questionBank = questionBank;
    this.algorithmConfig = config || this.getDefaultConfig();
    this.initializeProbabilities();
    this.testSession = this.createNewSession();
  }
  
  // 自适应测试会话
  private createNewSession(): AdaptiveTestSession {
    return {
      id: this.generateSessionId(),
      start_time: Date.now(),
      current_phase: 'exploration',
      questions_answered: 0,
      target_questions: 10,
      answered_question_ids: [],
      phase_transitions: [],
      confidence_history: [],
      information_gain_history: []
    };
  }
  
  // 初始化均匀概率分布
  private initializeProbabilities(): void {
    const uniformProb = 1 / 8; // 8种类型均匀分布
    this.currentProbabilities = {
      inner_motivation: {
        qian: uniformProb, kun: uniformProb, zhen: uniformProb, xun: uniformProb,
        kan: uniformProb, li: uniformProb, gen: uniformProb, dui: uniformProb
      },
      outer_behavior: {
        qian: uniformProb, kun: uniformProb, zhen: uniformProb, xun: uniformProb,
        kan: uniformProb, li: uniformProb, gen: uniformProb, dui: uniformProb
      }
    };
  }
  
  // 选择下一个最优题目
  selectNextQuestion(answeredQuestions: string[]): Question | null {
    const availableQuestions = this.questionPool.filter(
      q => !answeredQuestions.includes(q.id)
    );
    
    if (availableQuestions.length === 0) return null;
    
    // 计算每个题目的信息增益
    let bestQuestion: Question | null = null;
    let maxInformationGain = -1;
    
    for (const question of availableQuestions) {
      const informationGain = this.calculateInformationGain(question);
      if (informationGain > maxInformationGain) {
        maxInformationGain = informationGain;
        bestQuestion = question;
      }
    }
    
    return bestQuestion;
  }
  
  // 计算信息增益
  private calculateInformationGain(question: Question): number {
    const currentEntropy = this.calculateEntropy(
      question.category === 'inner_motivation' 
        ? this.currentProbabilities.inner_motivation
        : this.currentProbabilities.outer_behavior
    );
    
    let expectedEntropy = 0;
    
    // 计算每个选项的期望熵
    for (const option of question.options) {
      const updatedProbs = this.simulateUpdate(question.category, option);
      const optionEntropy = this.calculateEntropy(updatedProbs);
      expectedEntropy += (1 / question.options.length) * optionEntropy;
    }
    
    return currentEntropy - expectedEntropy;
  }
  
  // 计算熵值
  private calculateEntropy(probabilities: ProbabilityMap): number {
    let entropy = 0;
    for (const prob of Object.values(probabilities)) {
      if (prob > 0) {
        entropy -= prob * Math.log2(prob);
      }
    }
    return entropy;
  }
  
  // 更新概率分布
  updateProbabilities(question: Question, selectedOption: QuestionOption): void {
    const category = question.category;
    const currentProbs = category === 'inner_motivation' 
      ? this.currentProbabilities.inner_motivation
      : this.currentProbabilities.outer_behavior;
    
    // 贝叶斯更新
    const updatedProbs: ProbabilityMap = {} as ProbabilityMap;
    let totalWeight = 0;
    
    for (const [type, currentProb] of Object.entries(currentProbs)) {
      const coefficient = selectedOption.impact_coefficients[type as keyof ProbabilityMap];
      const likelihood = Math.exp(coefficient); // 将系数转换为似然度
      const newWeight = currentProb * likelihood;
      updatedProbs[type as keyof ProbabilityMap] = newWeight;
      totalWeight += newWeight;
    }
    
    // 归一化
    for (const type of Object.keys(updatedProbs)) {
      updatedProbs[type as keyof ProbabilityMap] /= totalWeight;
    }
    
    if (category === 'inner_motivation') {
      this.currentProbabilities.inner_motivation = updatedProbs;
    } else {
      this.currentProbabilities.outer_behavior = updatedProbs;
    }
  }
  
  // 获取最终结果
  getFinalResult(): { innerType: string; outerType: string; confidence: number } {
    const innerType = this.getMaxProbabilityType(this.currentProbabilities.inner_motivation);
    const outerType = this.getMaxProbabilityType(this.currentProbabilities.outer_behavior);
    
    const innerConfidence = Math.max(...Object.values(this.currentProbabilities.inner_motivation));
    const outerConfidence = Math.max(...Object.values(this.currentProbabilities.outer_behavior));
    const confidence = (innerConfidence + outerConfidence) / 2;
    
    return { innerType, outerType, confidence };
  }
  
  private getMaxProbabilityType(probabilities: ProbabilityMap): string {
    return Object.entries(probabilities).reduce((a, b) => 
      probabilities[a[0] as keyof ProbabilityMap] > probabilities[b[0] as keyof ProbabilityMap] ? a : b
    )[0];
  }
  
  // 智能题目选择算法
  selectOptimalQuestion(): ExtendedQuestion | null {
    const availableQuestions = this.getAvailableQuestions();
    if (availableQuestions.length === 0) return null;
    
    // 根据当前阶段选择策略
    switch (this.testSession.current_phase) {
      case 'exploration':
        return this.selectExplorationQuestion(availableQuestions);
      case 'discrimination':
        return this.selectDiscriminationQuestion(availableQuestions);
      case 'confirmation':
        return this.selectConfirmationQuestion(availableQuestions);
      default:
        return this.selectByInformationGain(availableQuestions);
    }
  }
  
  // 探索阶段：选择高区分度的通用题目
  private selectExplorationQuestion(questions: ExtendedQuestion[]): ExtendedQuestion {
    return questions
      .filter(q => q.test_phase === 'exploration' || q.test_phase === 'any')
      .sort((a, b) => b.information_value - a.information_value)[0];
  }
  
  // 区分阶段：选择能区分当前最可能类型的题目
  private selectDiscriminationQuestion(questions: ExtendedQuestion[]): ExtendedQuestion {
    const topTypes = this.getTopProbabilityTypes(2);
    const targetQuestions = questions.filter(q => 
      q.discrimination_targets.some(target => topTypes.includes(target))
    );
    
    return targetQuestions.length > 0 
      ? this.selectByInformationGain(targetQuestions)
      : this.selectByInformationGain(questions);
  }
  
  // 确认阶段：选择高置信度验证题目
  private selectConfirmationQuestion(questions: ExtendedQuestion[]): ExtendedQuestion {
    const mostLikelyType = this.getMostLikelyType();
    const confirmationQuestions = questions.filter(q => 
      q.test_phase === 'confirmation' && 
      q.discrimination_targets.includes(mostLikelyType)
    );
    
    return confirmationQuestions.length > 0
      ? confirmationQuestions.sort((a, b) => b.success_rate - a.success_rate)[0]
      : this.selectByInformationGain(questions);
  }
  
  // 阶段转换逻辑
  private checkPhaseTransition(): void {
    const currentConfidence = this.calculateOverallConfidence();
    const currentEntropy = this.calculateCurrentEntropy();
    
    if (this.algorithmConfig.phase_transition_strategy === 'adaptive') {
      if (this.testSession.current_phase === 'exploration' && 
          currentConfidence > this.algorithmConfig.confidence_threshold * 0.6) {
        this.transitionToPhase('discrimination', 'confidence_threshold_reached');
      } else if (this.testSession.current_phase === 'discrimination' && 
                 currentEntropy < this.algorithmConfig.entropy_threshold) {
        this.transitionToPhase('confirmation', 'entropy_threshold_reached');
      }
    } else {
      // 固定阶段转换
      if (this.testSession.questions_answered === this.algorithmConfig.exploration_questions) {
        this.transitionToPhase('discrimination', 'fixed_phase_schedule');
      } else if (this.testSession.questions_answered === 
                 this.algorithmConfig.exploration_questions + this.algorithmConfig.discrimination_questions) {
        this.transitionToPhase('confirmation', 'fixed_phase_schedule');
      }
    }
  }
}
```

## 6. 性能优化与缓存策略

### 6.1 算法性能优化

```typescript
class AdaptiveTestOptimizer {
  private informationGainCache: Map<string, number>;
  private probabilityUpdateCache: Map<string, ProbabilityMap>;
  private questionSelectionWorker: Worker;
  
  constructor() {
    this.informationGainCache = new Map();
    this.probabilityUpdateCache = new Map();
    this.initializeWebWorker();
  }
  
  // 预计算信息增益矩阵
  async precomputeInformationGains(questionBank: QuestionBank): Promise<void> {
    const computationTasks = [];
    
    for (const question of [...questionBank.inner_motivation_questions, ...questionBank.outer_behavior_questions]) {
      for (let i = 0; i < 256; i++) { // 2^8 种可能的概率状态
        const probabilityState = this.indexToProbabilityState(i);
        const cacheKey = `${question.id}_${i}`;
        
        computationTasks.push(
          this.computeInformationGainAsync(question, probabilityState)
            .then(gain => this.informationGainCache.set(cacheKey, gain))
        );
      }
    }
    
    await Promise.all(computationTasks);
  }
  
  // 使用Web Worker进行并行计算
  private async computeInformationGainAsync(
    question: ExtendedQuestion, 
    probabilityState: ProbabilityMap
  ): Promise<number> {
    return new Promise((resolve) => {
      this.questionSelectionWorker.postMessage({
        type: 'COMPUTE_INFORMATION_GAIN',
        question,
        probabilityState
      });
      
      this.questionSelectionWorker.onmessage = (event) => {
        if (event.data.type === 'INFORMATION_GAIN_RESULT') {
          resolve(event.data.gain);
        }
      };
    });
  }
  
  // 智能缓存策略
  getCachedInformationGain(question: ExtendedQuestion, currentProbs: ProbabilityMap): number | null {
    const stateIndex = this.probabilityStateToIndex(currentProbs);
    const cacheKey = `${question.id}_${stateIndex}`;
    return this.informationGainCache.get(cacheKey) || null;
  }
}
```

### 6.2 数据存储优化

```typescript
class AdaptiveTestStorage {
  private indexedDB: IDBDatabase;
  private compressionWorker: Worker;
  
  // 压缩存储测试会话数据
  async saveTestSession(session: AdaptiveTestSession): Promise<void> {
    const compressedData = await this.compressSessionData(session);
    const transaction = this.indexedDB.transaction(['test_sessions'], 'readwrite');
    const store = transaction.objectStore('test_sessions');
    await store.put(compressedData);
  }
  
  // 增量更新概率分布
  async updateProbabilityDistribution(
    sessionId: string, 
    newProbabilities: { inner_motivation: ProbabilityMap; outer_behavior: ProbabilityMap }
  ): Promise<void> {
    const transaction = this.indexedDB.transaction(['probability_history'], 'readwrite');
    const store = transaction.objectStore('probability_history');
    
    await store.put({
      session_id: sessionId,
      timestamp: Date.now(),
      probabilities: newProbabilities,
      compressed: true
    });
  }
}
```

### 5.2 卦象映射算法

```typescript
class HexagramMapper {
  private hexagramDatabase: HexagramData[];
  
  constructor(hexagramDatabase: HexagramData[]) {
    this.hexagramDatabase = hexagramDatabase;
  }
  
  // 根据内外类型映射到64卦
  mapToHexagram(innerType: string, outerType: string): HexagramData {
    const hexagramCode = this.generateHexagramCode(innerType, outerType);
    const hexagram = this.hexagramDatabase.find(h => h.code === hexagramCode);
    
    if (!hexagram) {
      throw new Error(`未找到对应卦象: ${innerType} + ${outerType}`);
    }
    
    return hexagram;
  }
  
  // 生成卦象编码
  private generateHexagramCode(innerType: string, outerType: string): string {
    const typeToTrigram: Record<string, string> = {
      'qian': '111', 'dui': '110', 'li': '101', 'zhen': '100',
      'xun': '011', 'kan': '010', 'gen': '001', 'kun': '000'
    };
    
    const lowerTrigram = typeToTrigram[innerType];
    const upperTrigram = typeToTrigram[outerType];
    
    return upperTrigram + lowerTrigram; // 6位二进制编码
  }
  
  // 生成分层分析
  generateLayeredAnalysis(
    hexagram: HexagramData, 
    probabilities: { inner_motivation: ProbabilityMap; outer_behavior: ProbabilityMap }
  ): {
    layer1_basic: BasicAnalysis;
    layer2_detailed: DetailedAnalysis;
    layer3_related: RelatedAnalysis;
  } {
    return {
      layer1_basic: this.generateBasicAnalysis(hexagram, probabilities),
      layer2_detailed: this.generateDetailedAnalysis(hexagram, probabilities),
      layer3_related: this.generateRelatedAnalysis(hexagram)
    };
```

